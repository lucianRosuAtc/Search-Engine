...
Sorry , we couldn&apos;t find any matches for{' '}<span className="text-geen-600 font-medium">{query}</span>
  ...
  return (
    <ul className="py-4 divide-y divide-zink-100 bg-white shadow-md rounded-b-md">
      {products.slice(0.3).map((product) => (
        <Link href={`/products/${product.id}`} key={product.id}>
          <li className="flex mx-auto px-8 py-4 space-x-4">
            <div className="relative flex items-center bg-zink-100 rounded-lg h-40 w-40">
              <Image
                src={`/${product.imageId}`}
                loading="eager"
                fill
          ...
          <h1 className="font-medium text-lg text-gray-900">{product.name}</h1>
          <p className="prose prose-sm text-gray-500 line-clamp-3">{product.description}</p>
          <p className="font-medium text-gray-900 text-base">${product.price.toFixed(2)}</p>
            </div>
          </li>
        </Link>
      ))}
